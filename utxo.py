...
from utils import change_endianness, encode_varint
    ...
    for key, o_value in db.iterator(prefix=prefix):
        ...
        if decode:
            if version < 0.15:
                ...
                value = decode_utxo(value, hex_key, version)

        fout.write(ujson.dumps({"key": bytes.decode(hexlify(key[2:33][::-1])), "value": value, "len": serialized_length}, sort_keys=True) + "\n")
...
if __name__ == '__main__':
        fin_name = os.path.join(os.getenv('HOME'), '.bitcoin', 'chainstate')
        fout_name = 'chainstate.txt'
        parse_ldb(fout_name, fin_name)
